...
class CvsController extends Controller
{
    ...
    function getBrandCvs(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            if ($request->get('status') == 'Disabled') {
                $data->where('tbl_cvs.is_active', 1);
            } else {
                if ($request->get('status') == 'Published') {
                    $data->where('tbl_cvs.status', 1);
                } elseif ($request->get('status') == 'Unpublished') {
                    $data->where('tbl_cvs.status', 0);
                }
            }
                ...
                try
                {
                    if($request->hasfile($audio_name))
                    {
                        if($request->$audio_name != '')
                        {
                            ...
                            DB::table('tbl_cv_block_6_data')->upsert($block_6_data, ['b6_id','cv_id'], ['b6_title','b6_name','cv_id','assets_id','cs_status','is_active','created_by','edited_by']);
                            //return redirect('brand-cvs')->with('success','Brand CV data updated successfully');
                        }
                    }
                    else
                ...
                if($currnt_cv_logo->cv_logo != '' && $currnt_cv_logo->cv_logo != null)
                {
                    ...
                    if (File::copy(public_path($oldPath) , public_path($newPathWithName)))
                    {
                        if(DB::table('tbl_cvs')->where('cv_id', $id)->update(['cv_logo' => $newName, 'is_active' => '0', 'edited_by'=>session('LoggedUser')]))
                        {
                            ...
                            $newMediumPathWithName = '/images/cv_logos/medium/'.$newMediumName;
    ...
    function addCvToProcessQueue($id)
    {
        ...
        $chk_data = DB::table('tbl_social_media_sync_process_data')->where('cv_id', $cv_id)->where('is_active', '=', 0)->first();
            ...
            foreach($get_sm_data_flag as $get_sm_data_flag_data)
            {
                if($get_sm_data_flag_data->process_type == 'youtube')
                {
                    if($get_sm_data_flag_data->status == 2)
                    {
                        ...
                        if($yt != 0)
                        {
                            $get_pending = DB::table('tbl_social_spyder_graph_meta_data')->where('cv_id',$cv_id)->where('is_active', '=', 0)->where('status', '>', 1)->where('process_type', '=', 'youtube')->orderBy('updated_at','desc')->first();
                            if($get_pending != '')
                            {
                ...
                if($get_sm_data_flag_data->process_type == 'instagram')
                {
                    if($get_sm_data_flag_data->status == 2)
                    {
                        ...
                        if($ig != 0)
                        {
                            ...
                            $get_pending = DB::table('tbl_social_spyder_graph_meta_data')->where('cv_id',$cv_id)->where('is_active', '=', 0)->where('status', '>', 1)->where('process_type', '=', 'instagram')->orderBy('updated_at','desc')->first();
                            if($get_pending != '')
                            {
                                $last_process_date = strtotime(explode(" ",$get_pending->updated_at)[0]);
                                if($end_date > $last_process_date)
                ...
                if($get_sm_data_flag_data->process_type == 'tiktok')
                {
                    if($get_sm_data_flag_data->status == 2)
                    {
                        ...
                        if($tt != 0)
                        {
                            ...
                            $get_pending = DB::table('tbl_social_spyder_graph_meta_data')->where('cv_id',$cv_id)->where('is_active', '=', 0)->where('status', '>', 1)->where('process_type', '=', 'tiktok')->orderBy('updated_at','desc')->first();
                            if($get_pending != '')
                            {
                                $last_process_date = strtotime(explode(" ",$get_pending->updated_at)[0]);
                                if($end_date > $last_process_date)
                ...
                if($get_sm_data_flag_data->process_type == 'twitter')
                {
                    if($get_sm_data_flag_data->status == 2)
                    {
                        ...
                        if($twt != 0)
                        {
                            ...
                            $get_pending = DB::table('tbl_social_spyder_graph_meta_data')->where('cv_id',$cv_id)->where('is_active', '=', 0)->where('status', '>', 1)->where('process_type', '=', 'twitter')->orderBy('updated_at','desc')->first();
                            if($get_pending != '')
                            {
                                $last_process_date = strtotime(explode(" ",$get_pending->updated_at)[0]);
                                if($end_date > $last_process_date)
    ...
    function publishCV(Request $request)
    {
        ...
        if($update_status_query)
        {
            if($request->status_type == "publish")
            {
                ...
                $response1 = $this->triggeraddCvToSocialBladeProcessQueue($request->cv_id);
