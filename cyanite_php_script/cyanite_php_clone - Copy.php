...
class cyanite_php_clone{
	...
	function upload_mp3s_on_cyanite(){
		...
		try{
			...
			error_log("get_cv_ids_qry for process_type ".$process_type." is ".$get_cv_ids_qry);
			...
			while($get_cv_ids_qry_res_row = $get_cv_ids_qry_res->fetch_assoc()) {
				...
				if($channel_count != 0)
				{
					if($channel_count > 1)
					{
						...
						error_log("Upload count for Multiple channels ".$upload_count);
						...
						foreach($process_type_array as $process_type)
						{
							...
							{
								$get_chnl_cnt_qry = "SELECT chn_id FROM ( SELECT * FROM `tbl_social_spyder_graph_request_data` WHERE `cv_id` = ".$get_cv_ids_qry_res_row['cv_id']." and `is_active` = 0 and `chn_notfound` = 0 and v_count != 0 and down_count != 0 and process_type = '".$process_type."' UNION SELECT * FROM `tbl_social_spyder_graph_request_data` WHERE `cv_id` = ".$get_cv_ids_qry_res_row['cv_id']." and `is_active` = 0 and `chn_notfound` = 0 and process_type = '".$process_type."' and v_count is null and down_count is null) ch";
								$get_chnl_cnt_qry_res = $conn->query($get_chnl_cnt_qry);

								while ($get_chnl_cnt_qry_res_row = $get_chnl_cnt_qry_res->fetch_assoc())
								{
									$get_first_chnl_cntnt_id_qry = "SELECT * FROM ( SELECT * FROM tbl_social_spyder_graph_meta_data WHERE cv_id=".$get_cv_ids_qry_res_row['cv_id']." and is_active=0 and chn_id ='".$get_chnl_cnt_qry_res_row['chn_id']."' LIMIT ".$upload_count.") sub ORDER BY id ASC limit 1";
									...
									$get_last_chnl_cntnt_id_qry = "SELECT * FROM ( SELECT * FROM tbl_social_spyder_graph_meta_data WHERE cv_id=".$get_cv_ids_qry_res_row['cv_id']." and is_active=0 and chn_id ='".$get_chnl_cnt_qry_res_row['chn_id']."' LIMIT ".$upload_count.") sub ORDER BY id DESC limit 1";
									...
									$get_chnl_cntnt_total_count_qry = "SELECT * FROM tbl_social_spyder_graph_meta_data WHERE cv_id = ".$get_cv_ids_qry_res_row['cv_id']." and chn_id ='".$get_chnl_cnt_qry_res_row['chn_id']."' and is_active=0";
									...
									/*else
									{
										$chnl_cntnt_upld_range["start_id"] = -1;
										$chnl_cntnt_upld_range["end_id"] = -1;
										$chnl_cntnt_upld_range["total_count"] = 0;
										$chnl_cntnt_upld_range_array[$get_chnl_cnt_qry_res_row['chn_id']] = $chnl_cntnt_upld_range;
										array_push($available_chnl_id_arr,$get_chnl_cnt_qry_res_row['chn_id']);
									}*/
					...
					else
					{
						...
						error_log("Upload count for Singel channel ".$upload_count);
