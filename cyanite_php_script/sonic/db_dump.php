	...
	function fetch_and_dump_analised_record($track_id, $brand_id, $start_date, $end_date, $matadata_id, $c_date,$chnl_id,$process_type){		
		...
		try{
			...
			if ($analysis_status == "AudioAnalysisV6Finished") {
				...
				if($json_str ->data ->libraryTrack ->audioAnalysisV6 ->result ->voiceoverExists === false)
				{
					$voiceover_exists = 0;
				}
				else
				...
				$sql = "INSERT INTO cyanite (`brand_id`, `LTrack_id`, `mood`, `moodtags`, `genre`, `genretags`, `voice`, `valence`, `arousal`, `bpm`, `key`, `voiceover_exists`, `voiceover_degree`, `segmentdetails`, `cyanitejson`, `status`, `version`, `start_date`, `end_date`,`c_date`,`chn_id`,`process_type`)
				VALUES (".$brand_id.",".$LTrack_id.",'".$mood."','".$moodtags."','".$genre."','".$genretags."','".$voice."','".$valence."','".$arousal."','".$bpm."','".$key."','".$voiceover_exists."','".$voiceover_degree."','".$segmentdetails."','".$cyanitejson."','".$status."',".$version.",'".$start_date."','".$end_date."','".$c_date."','".$chnl_id."','".$process_type."')";
